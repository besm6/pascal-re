...
//   KV1M =      &3500000;
    ...
    I7 =        &34000000;      (* frame pointer *)
    I8 =        &40000000;      (* const pointer *)
    I9 =        &44000000;      (* temp register *)
    I10 =       &50000000;      (* temp register *)
    I11 =       &54000000;      (* temp register *)
    I12 =       &60000000;      (* temp register *)
    I13 =       &64000000;      (* link register *)
    I14 =       &70000000;      (* temp register *)
    SP =        &74000000;      (* stack pointer, reg 15 *)
...
charmap   = packed array [Char(0)..Char(&176)] of char;
textmap   = packed array [Char(&052)..Char(&177)] of Char(0)..Char(&077);

four      = array [1..4] of integer;
entries   = array [1..42] of bitset;

expr = record
    case operator_ of (* arbitrary so far *)
    NOOP:       (val:   word;
                 op:    operator_;
                 d1,
                 d2:    word);
...
(*327-331*) iMulOpMap: array [MUL..IMODOP] of operator_;
(*332-338*) setOpMap: array [MUL..MINUSOP] of operator_;
(*339-340*) iAddOpMap: array [PLUSOP..MINUSOP] of operator_;
...
(*739*)     chrClassTabBase: array [Char(0)..Char(&177)] of operator_;
(*867*)     kwordHashTabBase: array [0..127] of ^kword;
(*995*)     charSymTabBase: array [Char(0)..Char(&177)] of symbol;
...
(*1577-
  2409*)    symTab: array [&74000..&75500] of bitset;
...
(*4675*)    iso2text: array [Char(&052)..Char(&177)] of Char(0)..Char(&077);
...
{TODO: type
    errchar = Char(0)..Char(&077);
    errtxt = packed array [0..100] of errchar;
    perrtxt = ^errtxt;
var
    errptr: ^errtxt;
    errtext: array [0..100] of errchar;
    i: integer;
    c: char;
    function pasmitxt(errno: integer): perrtxt;
        external;
    function pasisoxt(txtchar: errchar): char;
        external;
}
begin (* PrintErrMsg *)
    write(' ');
...
procedure printTextWord(val: word);
    ...
    {TODO: procedure PASTPR(val: word);
        external;}
...
2233:                       with PASINFOR do begin
                                if charEncoding = 3 then begin
                                    if (ch < '*') or (Char(&176) < CH) then
                                        curChar := chr(0)
                                    else begin
                                        curChar := iso2text[CH];
                                        (*=z-*)besm(3042246B);(*=z+*)
                                    end
                                end else if Char(&176) < CH then begin
                                    curChar := CH;
...
var
    ...
    l5inl20z: ^insnltyp;
    l5op21z: operator_; l5idc22z: idclass;
...
begin (* initTables *)
    ...
    initInsnTemplates;
    initSets;
    unpack(pasinfor.a3^, iso2text, Char(&052)); (* '*' *)
    rewrite(CHILD);
    for jdx to 10 do
        put(CHILD);
